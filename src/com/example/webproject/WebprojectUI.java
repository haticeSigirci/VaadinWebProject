...
@Theme("demo")
public class WebprojectUI extends UI {

	@WebServlet(value = { "/*", "/VAADIN/*" }, asyncSupported = true)
	@VaadinServletConfiguration(productionMode = false, ui = WebprojectUI.class, widgetset = "com.example.webproject.widgetsets.DemoWidgetSet")
	...
	private GoogleMapInfoWindow kakolaInfoWindow = new GoogleMapInfoWindow("Kakola used to be a provincial prison.",
			kakolaMarker);
	...
	protected void init(VaadinRequest request) {
		...
		googleMap.addMarker("DRAGGABLE: Paavo Nurmi Stadion", new LatLon(60.442423, 22.26044), true,
		...
		OpenInfoWindowOnMarkerClickListener infoWindowOpener = new OpenInfoWindowOnMarkerClickListener(googleMap,
		...
		Button addPolyOverlayButton = new Button("Add overlay over Luonnonmaa", new Button.ClickListener() {
			@Override
			public void buttonClick(ClickEvent event) {
				...
				GoogleMapPolygon overlay = new GoogleMapPolygon(points, "#ae1f1f", 0.8, "#194915", 0.5, 3);
		...
		Button addPolyLineButton = new Button("Draw line from Turku to Raisio", new Button.ClickListener() {
			@Override
			public void buttonClick(ClickEvent event) {
				ArrayList<LatLon> points = new ArrayList<LatLon>();
		...
		Button addPolyLineButton2 = new Button("Draw line from Turku to Raisio2", new Button.ClickListener() {
			@Override
			public void buttonClick(ClickEvent event) {
		...
		Button changeToTerrainButton = new Button("Change to terrain map", new Button.ClickListener() {
			@Override
			public void buttonClick(ClickEvent event) {
		...
		Button addInfoWindowButton = new Button("Add InfoWindow to Kakola marker", new Button.ClickListener() {
			@Override
			public void buttonClick(ClickEvent event) {
		...
		Button moveMarkerButton = new Button("Move kakola marker", new Button.ClickListener() {
